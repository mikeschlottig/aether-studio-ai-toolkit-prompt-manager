...
import { updateSessionActivity, getAppController } from './core-utils';
...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean }): Promise<Response> {
    ...
    try {
      ...
      // SYNC MCP SERVERS: Fetch latest dynamic tools from control plane
      const controller = getAppController(this.env);
      const registeredServers = await controller.getMCPServers();
      await mcpManager.setServers(registeredServers);
